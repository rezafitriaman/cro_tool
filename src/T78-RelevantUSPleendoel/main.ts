...
class V1 extends Base {
    ...
    @TryAndCatch
    customCode() {
        let newUsp = document.querySelectorAll('.space-bottom');

        if(newUsp.length < 1 ) {
            (<HTMLElement>this.targets[0]).insertAdjacentHTML('beforebegin', this.uspString);
        }
    }
}
