...
var utility_module_1 = require("./custom_module/utility.module");
var mainCode_module_1 = require("./custom_module/mainCode.module");
/*
==============================
REQUIRED
==============================
*/
var gulp = require('gulp'), uglify = require('gulp-uglify'), bs = require('browser-sync').create(), reload = bs.reload, plumber = require('gulp-plumber'), ts = require('gulp-typescript'), tsProject = ts.createProject("tsconfig.json"), replace = require('gulp-string-replace'), browserify = require("browserify"), source = require('vinyl-source-stream'), tsify = require("tsify"), watchify = require("watchify"), sourcemaps = require('gulp-sourcemaps'), filePush = require('gulp-file'), //need to create file
fileExists = require('file-exists'), buffer = require('vinyl-buffer'), meta_data = require('./meta_data_CRO.ts'); //custom meta data form-CRO
/*
==============================
PATHS - REWRITEFILE
==============================
...
gulp.task('create::utilityModule', function () {
    return filePush('utility.ts', utility_module_1.utility(), { src: true })
        .pipe(gulp.dest("src/" + meta_data.testCRO.id));
});
gulp.task('create::mainCodeModule', function () {
    console.log('create main.ts file!');
    return filePush('main.ts', mainCode_module_1.mainCode(), { src: true })
        .pipe(gulp.dest("src/" + meta_data.testCRO.id));
});
...
gulp.task('build::modules', function () {
    fileExists("src/" + meta_data.testCRO.id + "/main.ts").then(function (exists) {
        if (exists) {
